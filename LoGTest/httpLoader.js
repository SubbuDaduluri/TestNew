      ...
      return {
        ...
        link: function ($scope) {
          ...
          var toggleShowLoader = function (event, method) {
        	//Added by sujit starts here
        	//secondsBeforeExpire=sessionStorage.getItem("sessionExpiryTime");
        	updateTimer();
        	/*$("tbody tr").click(function() {
        	    console.log('clicked');
        	   // $("tbody tr").removeAttr("style")
        	  // $(this).css({"background-color": "#2192ab", "color": "white"});
        	    $("tbody tr").removeClass("tablehighlighter");
        		$(this).addClass("tablehighlighter");
        	});*/
            ...
            if (methods.indexOf(method.toUpperCase()) !== -1) {
             ...
             if(method == "customLoader"){
            	showLoader = (event.name === 'loaderShow');
            }
            if(method == "customLoader"){
            	if(event.name === 'loaderHide'){
            		showLoader = false;
            	}
            }
